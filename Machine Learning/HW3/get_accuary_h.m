function [accuracy, train_set, train_label, alpha]= get_accuary_h(X, Y, lambda, h, hold_out_rate, seed)
    len = length(X(:, 1));
    ...
    accuracy.overall = sum(preds == test_label)/length(test_label); 
    accuracy.truePositive = sum((preds == 1) == (labels == 1))/len;
    accuracy.trueNegative = sum((preds == 0) == (labels == 0))/len;
    accuracy.falsePositive = sum((preds == 1) == (labels == 0))/len;
    accuracy.falseNegative = sum((preds == 0) == (labels == 1))/len;     
end
